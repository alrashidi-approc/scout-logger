...
class SharedReportView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Column (not ListView) — embedded in parent scroll views (Reports, share).
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (report.snapshotUrl != null) ...[
            ...
            Text('Full snapshot link included in exported PDF.',
                style: TextStyle(
  ...
  Widget _verdictBanner(Report report) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
            child: Text(report.verdictLabel!,
                style: TextStyle(
            ...
            for (final h in items)
              Padding(
                ...
                child: Row(
                    ...
                    children: [
                      Text('• ',
                          style: TextStyle(color: _severityColor(h.severity))),
                      Expanded(
                          child: Text(h.text,
  ...
  Widget _goNoGoChip(String value) {
    ...
    return Align(
      ...
      child: Chip(
        label: Text('Release signal: $label',
  ...
  List<Widget> _section(BuildContext context, ReportSection s) => [
        ...
        for (final c in s.charts)
          if (!c.isEmpty) ...[
            const SizedBox(height: 12),
            DashboardPanel(
                title: c.title, child: SizedBox(height: 220, child: _chart(c))),
          ],
  ...
  Widget _table(BuildContext context, ReportTable t) => SingleChildScrollView(
        ...
        child: DataTable(
          ...
          headingTextStyle: const TextStyle(
              fontWeight: FontWeight.w700, fontSize: 12, color: AppTheme.muted),
              ...
              DataRow(cells: [
                for (var i = 0; i < row.cells.length; i++)
                  DataCell(
                    i == 0 &&
                            (row.linkUrl != null ||
                                (row.issueId != null && onIssueTap != null))
                        ? InkWell(
                            onTap: () {
                              if (row.linkUrl != null) {
  ...
  Widget _chart(ReportChart c) {
    ...
    Widget bottom(double value, TitleMeta meta) {
      final i = value.toInt();
      if (i < 0 || i >= c.xLabels.length || i % step != 0)
        return const SizedBox.shrink();
      return Padding(
        padding: const EdgeInsets.only(top: 6),
        child: Text(c.xLabels[i],
    ...
    final titles = FlTitlesData(
      ...
      bottomTitles: AxisTitles(
          sideTitles: SideTitles(
              showTitles: true, reservedSize: 26, getTitlesWidget: bottom)),
        ...
        lineBarsData: [
          for (var i = 0; i < c.series.length; i++)
            LineChartBarData(
              ...
              spots: [
                for (var x = 0; x < c.series[i].values.length; x++)
                  FlSpot(x.toDouble(), c.series[i].values[x])
              ],
            ),
        ],
      ));
    ...
    final values = c.series.isEmpty ? const <double>[] : c.series.first.values;
